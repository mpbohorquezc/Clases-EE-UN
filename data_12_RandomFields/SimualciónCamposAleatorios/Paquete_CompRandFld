...
data <- RFsim(x, y, corrmodel="matern", param=list(smooth=0.5,mean=0,sill=1,scale=0.2,nugget=0))
...
sim <- RFsim(x, y, corrmodel="exponential", grid=TRUE,model="BinaryGauss", threshold=0,param=list(nugget=0,mean=0,scale=.1,sill=1))
...
y <- seq(0, 1, 0.02)
# Simulation of a spatial binary random field:
sim <- RFsim(x, y, corrmodel="exponential", grid=TRUE, model="ExtT",numblock=500, param=list(nugget=0,mean=0,scale=.1,sill=1,df=5))
...
#
# Simulation of a spatial Gaussian random field:
sim <- RFsim(x, y, times, corrmodel="exp_exp", grid=TRUE,param=list(nugget=0,mean=0,scale_s=0.3,scale_t=0.5,sill=1))$data
...
distance="Geod",grid=TRUE,param=list(nugget=0,mean=0,scale=8000,sill=1))$data
